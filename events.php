...
include_once('eventarray.php');
include_once('event_classes.php');

function getEventsForDepartment($department) {
    $arr = [];
    global $events;
    foreach($events as $eventelement) {
        if($eventelement['event_class']==$department) {
            array_push($arr, $eventelement);
        }
    }
    return $arr;
}

function getDetailForParticularEvent($eventName) {
    global $events;
    foreach($events as $eventelement) {
        if($eventelement['eventID']==$eventName) {
            return $eventelement;
        }
    }
    return NULL;
...
function getEventClass($event) {
    ...
    foreach($event_classes as $eventt) {
        if($eventt['event_class']==$event)
            return $eventt;
    }
    return null;
}
...
<?php
  ...
  $events = getEventsForDepartment($_GET['event_class']);
?>
<section style="width:88%;margin:20px auto 0;padding:20px">
...
<?php
  foreach($events as $key=>$event){
    $eventClass = getEventClass($event['event_class']);
?>
...
<?php
  ...
  $event = getDetailForParticularEvent($_GET['event']);
  $eventClass = getEventClass($event['event_class']);
?>
<section style="width:88%;margin:20px auto 0;padding:20px">
